---@class lvim-shell-modules.win
---@field id number
---@field buf? number
---@field scratch_buf? number
---@field win? number
---@field opts lvim-shell-modules.win.Config
---@field augroup? number
---@field backdrop? lvim-shell-modules.win
---@field keys lvim-shell-modules.win.Keys[]
---@field events (lvim-shell-modules.win.Event|{event:string|string[]})[]
---@field meta table<string, string>
---@field closed? boolean
---@overload fun(opts? :lvim-shell-modules.win.Config|{}): lvim-shell-modules.win
...
---@class lvim-shell-modules.win.Keys: vim.api.keyset.keymap
---@field [1]? string
---@field [2]? string|string[]|fun(self: lvim-shell-modules.win): string?
---@field mode? string|string[]

---@class lvim-shell-modules.win.Event: vim.api.keyset.create_autocmd
---@field buf? true
---@field win? true
---@field callback? fun(self: lvim-shell-modules.win, ev:vim.api.keyset.create_autocmd.callback_args):boolean?

---@class lvim-shell-modules.win.Backdrop
---@field bg? string
---@field blend? number
---@field transparent? boolean defaults to true
---@field win? lvim-shell-modules.win.Config overrides the backdrop window config

---@class lvim-shell-modules.win.Dim
...
---@alias lvim-shell-modules.win.Action.fn fun(self: lvim-shell-modules.win):(boolean|string?)
---@alias lvim-shell-modules.win.Action.spec lvim-shell-modules.win.Action|lvim-shell-modules.win.Action.fn
---@class lvim-shell-modules.win.Action
---@field action lvim-shell-modules.win.Action.fn
---@field desc? string

---@class lvim-shell-modules.win.Config: vim.api.keyset.win_config
---@field style? string merges with config from `LvimShellModules.config.styles[style]`
---@field show? boolean Show the window immediately (default: true)
---@field height? number|fun(self:lvim-shell-modules.win):number Height of the window. Use <1 for relative height. 0 means full height. (default: 0.9)
---@field width? number|fun(self:lvim-shell-modules.win):number Width of the window. Use <1 for relative width. 0 means full width. (default: 0.9)
...
---@field col? number|fun(self:lvim-shell-modules.win):number Column of the window. Use <1 for relative column. (default: center)
---@field row? number|fun(self:lvim-shell-modules.win):number Row of the window. Use <1 for relative row. (default: center)
...
---@field backdrop? number|false|lvim-shell-modules.win.Backdrop Opacity of the backdrop (default: 60)
...
---@field keys? table<string, false|string|fun(self: lvim-shell-modules.win)|lvim-shell-modules.win.Keys> Key mappings
---@field on_buf? fun(self: lvim-shell-modules.win) Callback after opening the buffer
---@field on_win? fun(self: lvim-shell-modules.win) Callback after opening the window
---@field on_close? fun(self: lvim-shell-modules.win) Callback after closing the window
...
---@field actions? table<string, lvim-shell-modules.win.Action.spec> Actions that can be used in key mappings
...
LvimShellModules.config.style("float", {
	position = "float",
	backdrop = 60,
	height = 0.9,
	width = 0.9,
...
LvimShellModules.config.style("help", {
	position = "float",
	backdrop = false,
	border = "top",
	row = -1,
...
LvimShellModules.config.style("split", {
	position = "bottom",
	height = 0.4,
	width = 0.4,
})

LvimShellModules.config.style("minimal", {
	wo = {
		cursorcolumn = false,
		cursorline = false,
		cursorlineopt = "both",
...
local SCROLL_UP, SCROLL_DOWN = LvimShellModules.util.keycode("<c-u>"), LvimShellModules.util.keycode("<c-d>")
...
--@private
---@param ...? lvim-shell-modules.win.Config|string|{}
---@return lvim-shell-modules.win.Config
function M.resolve(...)
	local done = {} ---@type table<string, boolean>
	local merge = {} ---@type lvim-shell-modules.win.Config[]
	local stack = {}
	for i = 1, select("#", ...) do
		local next = select(i, ...) ---@type lvim-shell-modules.win.Config|string?
		if next then
	...
	while #stack > 0 do
		...
		next = type(next) == "string" and LvimShellModules.config.styles[next] or next
		---@cast next lvim-shell-modules.win.Config?
...
---@param opts? lvim-shell-modules.win.Config|{}
---@return lvim-shell-modules.win
function M.new(opts)
	local self = setmetatable({}, M)
	id = id + 1
	self.id = id
	self.meta = {}
	opts = M.resolve(LvimShellModules.config.get("win", defaults), opts)
	...
	self.keys = {}
	self.events = {}
	local done = {} ---@type table<string, lvim-shell-modules.win.Keys>
	for key, spec in pairs(opts.keys) do
		if spec then
			...
			---@cast spec lvim-shell-modules.win.Keys
			local lhs = LvimShellModules.util.normkey(spec[1] or "")
	...
	---@cast opts lvim-shell-modules.win.Config
	self.opts = opts
...
---@param opts? {col_width?: number, key_width?: number, win?: lvim-shell-modules.win.Config}
function M:toggle_help(opts)
	...
	for _, keymap in ipairs(keys) do
		local key = LvimShellModules.util.normkey(keymap.lhs or "")
...
---@param cb fun(self: lvim-shell-modules.win, ev:vim.api.keyset.create_autocmd.callback_args):boolean?
---@param opts? lvim-shell-modules.win.Event
...
function M:close(opts)
	...
	try_close = function()
		...
		elseif not ok then
			vim.notify("Failed to close window: " .. tostring(err), vim.log.levels.ERROR, {
				title = "LVIM Shell Modules",
			})
		end
	end
...
function M:update()
	if self:valid() then
		LvimShellModules.util.bo(self.buf, self.opts.bo)
		LvimShellModules.util.wo(self.win, self.opts.wo)
		if self:is_floating() then
			local opts = self:win_opts()
			opts.noautocmd = nil
...
function M:show()
	...
	if LvimShellModules.util.is_transparent() then
...
function M:drop()
	...
	---@cast backdrop lvim-shell-modules.win.Backdrop

	if
		(LvimShellModules.util.is_transparent() and backdrop.transparent)
	...
	if not backdrop.transparent then
		if LvimShellModules.util.is_transparent() then
			...
			bg = LvimShellModules.util.blend(LvimShellModules.util.color("Normal", "bg"), bg, winblend / 100)
		end
		winblend = 0
	end
...
---@param parent? lvim-shell-modules.win.Dim
function M:dim(parent)
	parent = parent or self:parent_size()
	---@type lvim-shell-modules.win.Dim
	local ret = {
		height = 0,
		width = 0,
		col = 0,
	...
	---@param s? number|fun(win:lvim-shell-modules.win):number? size
	...
	---@param p? number|fun(win:lvim-shell-modules.win):number? pos
	---@param s number size
	---@param ps number parent size
